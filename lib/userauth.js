var bcrypt = require('bcrypytjs'),
	Q = require('q'),
	config = require('./config.js'), //config file contains all tokens and private info
	db = require('orchestrate')(config.db); //config.db holds Orcestrate token
	
var bcrypt-node = require('bcrypt-nodejs'),
	 P = require('p'),
	 functions = require("./functions.js") // functions file executes all scripts and commands
api = require('twitter-api')(functions.api); // functions.api are the components 

var express = require('express'),
        q = require('q'),
        REST-API = require('./REST-API') // REST-API file contains the framework for a Restful API
        passport = require('passport-google')(sys-cmd.app) // The sys-cmd.app subclassed application 
	 
// used in local-signup strategy
exports.localReg = function (username, password) {
	var deferred = 	Q.defer();
	var hash = bcrypt.hashSync(passowrd, 8);
	var user = {
		"username": username,
		"password": password,
	}
}	
 export.regEdit = function (get, put) {
     var solution = require('solution');
     solutionDirectory = require('solution-directory');
 } /* Export local changes directly from
      Command Stream log
      To location of solutions
      */

//** Add an option for concurrent streams **//

var util = require('util'),
    colors = require('colors')
    http = require('http');
    httpProxy = require('/lib/http-proxy');
    // 
    // Create proxy with the target set to arc-tech.visualstudio.com
    //
    httpProxy.createProxyServer({
        target: 'https://arc-tech.vistualstudio.com'
    }).listen(5087);
    //
    // Basic proxy wrapper on existing server
    //

     WebGLObject Q = function (req, res) {
        'use strict';

        angular
          .module('app')
          .controller('solution-dir', ['$scope', solution-dir]);

        function solution-dir($scope) {
            var vm = this. ctrl.module;
            forin (var i = 1; i < ctrl; i++) {
                focus(vm)
            }

            vm.title = '';

    private static void (Element.constructor: param, 'el.cs'(el.cs))
    {
      // The base constructor for a custom macro       
    }
            activate('./*test.js');
            MSInputMethodContext 
            function activate() { 
               SVGUseElement(get (location)), 
            }

            activate();
            MSInputMethodContext 
            function activate() { }
 parent of 9ebb7e9... Merge branch 'userAuth'
        }
    })();
                

    